...
for i in range(10):
  ...
  mu_app = normalise_vector(student.policy_observation(trainedPolicy)) # all feature sums normalised
  
  delta_theta = fac*(mu_app-mu_expert) #other way round because we talk of errors
  ...
  print_dyn(str(['apprentice FE %s'% mu_app +'and expert FE %s' % mu_expert]))
