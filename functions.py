...
def extract_sa(oa):
  ...
  state = [0.0] * 9
  q = quaternion_from_orientation(array(oa[ROLL:AILR+12]).tolist())
  state[U:X] = rotate(array(oa[U:X]).tolist(), q)
  state[X:P] = rotate(array(oa[X:P]).tolist(), q)
  state[P:ROLL] = array(oa[P:ROLL]).tolist()
  action = array(oa[AILR+12:]).tolist()
